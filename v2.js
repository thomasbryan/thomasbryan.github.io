...
$(document).on('click','.branch',function() {
  ...
  if(!(todo&&func=="workread")) action({"u":"https://home.thomasbryan.info/project/","d":{"req":"read","branch":branch},"f":func});
});
...
function plan() {
  hide();
  $('#search, #nav .plan, #trunks').removeClass('hidden');
  $('#name').focus();
...
$(document).on('click touchend','#planning',function() {
  app.mode="plan";
  init();
});
/* Working */
...
function hide() {
  $('#content .nav, #search, #nav li,#work .btn-group-justified, #ip, #td').addClass('hidden');
}
